# ALYASMEEN AuntOps — Project Brief for Codex
...
The aunt manages orders via a built-in web dashboard. Codex Haiku powers the AI conversation.
...
- **AI:** Codex Haiku via Anthropic SDK — `app/services/ai_service.py`
...
│   │   ├── ai_service.py        # ONLY AI file — Codex Haiku, product context, chat history
...
| `chat_history` | AI conversation memory (last 6 turns to Codex) |
...
Customer sends WhatsApp message
  ...
  whatsapp.py  (hard commands first)
   ...
   2x1, 3*2   → add product with quantity
        │
   no match → ai_service.py → Codex Haiku (with 4 tools)
                │
                ...
                │
                Codex writes final reply knowing what the tools returned
```

**Agentic loop (2 API calls when a tool fires):**
1. First call → Codex picks a tool (stop_reason = "tool_use")
2. Tool executes in `whatsapp.py` (has full DB/session access)
3. Result sent back to Codex as tool_result
4. Second call → Codex writes the conversational reply

Hard commands always intercept before AI. If customer types `confirm`, `cart`, `clear`, etc. the hard handler runs and Codex is never called.
...
| `CLAUDE_MODEL` | optional | Default: Codex-haiku-4-5-20251001 |
